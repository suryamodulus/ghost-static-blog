...
const transformUrl = (url) => {
  return url.replace(process.env.GHOST_API_URL, "/blog");
};
...
module.exports = function (config) {
  ...
  config.addCollection("docs", async function (collection) {
    ...
    collection.map((doc) => {
      ...
      doc.primary_author.url = transformUrl(doc.primary_author.url);
  ...
  config.addCollection("posts", async function (collection) {
    ...
    collection.forEach((post) => {
      post.url = transformUrl(post.url);
      post.primary_author.url = transformUrl(post.primary_author.url);
      post.tags.map((tag) => (tag.url = transformUrl(tag.url)));
  ...
  config.addCollection("authors", async function (collection) {
    ...
    collection.forEach(async (author) => {
      ...
      author.url = transformUrl(author.url);
    });
  ...
  config.setBrowserSyncConfig({
    callbacks: {
      ready: (err, bs) => {
        const content_404 = fs.readFileSync("public/blog/404.html");
